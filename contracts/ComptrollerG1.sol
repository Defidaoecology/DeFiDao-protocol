 ...
 * @author DeFiDao
 * @dev This was the first version of the snktroller brains.
 *  We keep it so our tests can continue to do the real-life behavior of upgrading from this logic forward.
 */
contract snktrollerG1 is snktrollerV1Storage, snktrollerInterface, snktrollerErrorReporter, Exponential {
    struct Market {
        /**
    ...
    function _become(Unitroller unitroller, PriceOracle _oracle, uint _closeFactorMantissa, uint _maxAssets, bool reinitializing) public {
        ...
        if (!reinitializing) {
            snktrollerG1 freshBrainedsnktroller = snktrollerG1(address(unitroller));

            // Ensure invoke _setPriceOracle() = 0
            uint err = freshBrainedsnktroller._setPriceOracle(_oracle);
            require (err == uint(Error.NO_ERROR), "set price oracle error");

            // Ensure invoke _setCloseFactor() = 0
            err = freshBrainedsnktroller._setCloseFactor(_closeFactorMantissa);
            require (err == uint(Error.NO_ERROR), "set close factor error");

            // Ensure invoke _setMaxAssets() = 0
            err = freshBrainedsnktroller._setMaxAssets(_maxAssets);
            ...
            err = freshBrainedsnktroller._setLiquidationIncentive(liquidationIncentiveMinMantissa);
     ...
     * There should be no way to satisfy msg.sender == snktrollerImplementaiton
     * without tx.origin also being admin, but both are included for extra safety
     */
    function adminOrInitializing() internal view returns (bool) {
        bool initializing = (
                msg.sender == snktrollerImplementation
                &&
