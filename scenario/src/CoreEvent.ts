...
import { snktrollerCommands, processsnktrollerEvent } from './Event/snktrollerEvent';
import { processUnitrollerEvent, unitrollerCommands } from './Event/UnitrollerEvent';
import { snktrollerImplCommands, processsnktrollerImplEvent } from './Event/snktrollerImplEvent';
...
import { snkCommands, processsnkEvent } from './Event/snkEvent';
...
import { DeFiDaoLens } from './Contract/DeFiDaoLens';
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] = [
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Unitroller ...event" - Runs given Unitroller event
        * E.g. "Unitroller SetPendingImpl MysnktrollerImpl"
    `,
    'Unitroller',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "snktroller ...event" - Runs given snktroller event
        * E.g. "snktroller _setReserveFactor 0.5"
    `,
    'snktroller',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processsnktrollerEvent(world, event.val, from),
    { subExpressions: snktrollerCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "snktrollerImpl ...event" - Runs given snktrollerImpl event
        * E.g. "snktrollerImpl MyImpl Become"
    `,
    'snktrollerImpl',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processsnktrollerImplEvent(world, event.val, from),
    { subExpressions: snktrollerImplCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "snk ...event" - Runs given snk event
      * E.g. "snk Deploy"
    `,
    'snk',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processsnkEvent(world, event.val, from);
    },
    { subExpressions: snkCommands() }
  ),
  ...
  buildContractEvent<DeFiDaoLens>("DeFiDaoLens", false),
