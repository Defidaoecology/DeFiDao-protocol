...
export async function getUnitroller(world: World): Promise<snktroller> {
  return getWorldContract(world, [['Contracts', 'Unitroller']]);
}

export async function getMaximillion(world: World): Promise<snktroller> {
  return getWorldContract(world, [['Contracts', 'Maximillion']]);
}

export async function getsnktroller(world: World): Promise<snktroller> {
  return getWorldContract(world, [['Contracts', 'snktroller']]);
}

export async function getsnktrollerImpl(world: World, snktrollerImplArg: Event): Promise<snktrollerImpl> {
  return getWorldContract(world, [['snktroller', mustString(snktrollerImplArg), 'address']]);
}
...
export async function getsnk(
  world: World,
  snkArg: Event
): Promise<snk> {
  return getWorldContract(world, [['snk', 'address']]);
}

export async function getsnkData(
  world: World,
  snkArg: string
): Promise<[snk, string, Map<string, string>]> {
  let contract = await getsnk(world, <Event>(<any>snkArg));
  let data = getContractData(world, [['snk', snkArg]]);

  return [contract, snkArg, <Map<string, string>>(<any>data)];
}
...
export async function getsnktrollerImplData(
  world: World,
  snktrollerImplArg: string
): Promise<[snktrollerImpl, string, Map<string, string>]> {
  let contract = await getsnktrollerImpl(world, <Event>(<any>snktrollerImplArg));
  let data = getContractData(world, [['snktroller', snktrollerImplArg]]);

  return [contract, snktrollerImplArg, <Map<string, string>>(<any>data)];
}
