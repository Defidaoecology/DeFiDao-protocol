...
import { getsnktroller, getCTokenData } from '../ContractLookup';
...
async function setsnktroller(world: World, from: string, cToken: CToken, snktroller: string): Promise<World> {
  let invokation = await invoke(world, cToken.methods._setsnktroller(snktroller), from, CTokenErrorReporter);

  world = addAction(
    world,
    `Set snktroller for ${cToken.name} to ${snktroller} as ${describeUser(world, from)}`,
    invokation
  );
...
async function printLiquidity(world: World, cToken: CToken): Promise<World> {
  ...
  let snktroller = await getsnktroller(world);
...
export function cTokenCommands() {
  ...
  `,
    ...
    new Command<{ cToken: CToken, snktroller: AddressV }>(`
        #### Setsnktroller

        * "CToken <cToken> Setsnktroller snktroller:<Contract>" - Sets the snktroller for the given cToken
          * E.g. "CToken cZRX Setsnktroller snktroller"
      `,
      "Setsnktroller",
      [
        new Arg("cToken", getCTokenV),
      ...
      (world, from, { cToken, snktroller }) => setsnktroller(world, from, cToken, snktroller.val),
      { namePos: 1 }
    ),
    new Command<{
      cToken: CToken;
