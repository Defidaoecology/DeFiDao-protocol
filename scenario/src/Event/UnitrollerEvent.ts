...
import { getsnktrollerImpl, getUnitroller } from '../ContractLookup';
...
async function acceptAdmin(world: World, from: string, unitroller: Unitroller): Promise<World> {
  let invokation = await invoke(world, unitroller.methods._acceptAdmin(), from, snktrollerErrorReporter);
...
async function setPendingImpl(
  ...
  let invokation = await invoke(
    world,
    unitroller.methods._setPendingImplementation(snktrollerImpl._address),
    from,
    snktrollerErrorReporter
  );

  world = addAction(world, `Set pending snktroller impl to ${snktrollerImpl.name}`, invokation);
  ...
  return [
    ...
    new Command<{ unitroller: Unitroller; snktrollerImpl: snktrollerImpl }>(
      `
        ...
        * "SetPendingImpl impl:<Impl>" - Sets the pending snktroller implementation for this unitroller
          * E.g. "Unitroller SetPendingImpl MyScenImpl" - Sets the current snktroller implementation to MyScenImpl
      `,
      'SetPendingImpl',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
      ...
      (world, from, { unitroller, snktrollerImpl }) =>
        setPendingImpl(world, from, unitroller, snktrollerImpl)
    )
  ];
}
