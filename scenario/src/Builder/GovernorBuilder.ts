...
export async function buildGovernor(
  ...
  const fetchers = [
    new Fetcher<
      { name: StringV, timelock: AddressV, snk: AddressV, guardian: AddressV },
      GovernorData
    >(
      `
      #### GovernorAlpha

      * "Governor Deploy Alpha name:<String> timelock:<Address> snk:<Address> guardian:<Address>" - Deploys DeFiDao Governor Alpha
        * E.g. "Governor Deploy Alpha GovernorAlpha (Address Timelock) (Address snk) Guardian"
    `,
      "Alpha",
      [
        new Arg("name", getStringV),
        new Arg("timelock", getAddressV),
        new Arg("snk", getAddressV),
        new Arg("guardian", getAddressV)
      ],
      async (world, { name, timelock, snk, guardian }) => {
        return {
          invokation: await GovernorAlphaContract.deploy<Governor>(
            world,
            from,
            [timelock.val, snk.val, guardian.val]
          ),
          name: name.val,
    ...
    >(
      ...
      * "Governor Deploy AlphaHarness name:<String> timelock:<Address> snk:<Address> guardian:<Address>" - Deploys DeFiDao Governor Alpha with a mocked voting period
        * E.g. "Governor Deploy AlphaHarness GovernorAlphaHarness (Address Timelock) (Address snk) Guardian"
    `,
      "AlphaHarness",
      [
        new Arg("name", getStringV),
