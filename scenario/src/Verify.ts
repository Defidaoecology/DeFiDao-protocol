...
export async function verify(world: World, apiKey: string, contractName: string, buildInfoName: string, address: string): Promise<void> {
  ...
  let snkilerVersion: string = version.replace(/(\.Emscripten)|(\.clang)|(\.Darwin)|(\.appleclang)/gi, '');
  ...
  const verifyData: object = {
    ...
    snkilerversion: `v${snkilerVersion}`,
    optimizationUsed: '1',
    runs: '200',
    constructorArguements: constructorAbi.slice(2)
  };

  world.printer.printLine(`Verifying ${contractName} at ${address} with snkiler version ${snkilerVersion}...`);
