...
export interface Value {
  snkareTo(world: World, given: Value): boolean
  snkareOrder(world: World, given: Value): Order
  toString(): string
  truthy(): boolean
}

function snkareInt(a: number, b: number): Order {
  if (a === b) {
    return Order.EQUAL;
  } else if (a > b) {
...
export class EventV implements Value {
  ...
  snkareTo(world: World, given: Value): boolean {
    if (given instanceof EventV) {
      ...
      throw new Error(`Cannot snkare ${typeof this} to ${typeof given} (${this.toString()}, ${given.toString()})`);
    }
  }

  snkareOrder(world: World, given: Value): Order {
    throw new Error(`Cannot snkare order of ${typeof this} to ${typeof given} (${this.toString()}, ${given.toString()})`);
  }
...
export class BoolV implements Value {
  ...
  snkareTo(world: World, given: Value): boolean {
    if (given instanceof BoolV) {
      ...
      return this.snkareTo(world, given.toBoolV());
...
export class NumberV implements Value {
  ...
  snkareTo(world: World, given: Value): boolean {
    if (given instanceof NumberV) {
      ...
      return this.snkareTo(world, given.toNumberV());
    } else if (given instanceof StringV) {
      return this.snkareTo(world, new NumberV(Number(given.val)));
    } else {
      throw new Error(`Cannot snkare ${typeof this} to ${typeof given} (${this.toString()}, ${given.toString()})`);
    }
  }
  ...
  snkareOrder(world: World, given: Value): Order {
    if (given instanceof NumberV) {
      ...
      return snkareInt(thisBig, givenBig);
    } else if (given instanceof PreciseV) {
      return this.snkareOrder(world, given.toNumberV());
    } else {
      throw new Error(`Cannot snkare ${typeof this} to ${typeof given} (${this.toString()}, ${given.toString()})`);
    }
  }
...
export class ListV implements Value {
  ...
  snkareTo(world: World, given: Value): boolean {
    if (given instanceof ListV || given instanceof ArrayV) {
      return this.val.every((el, i) => el.snkareTo(world, given.val[i] || new NothingV()));
    } else {
      throw new Error(`Cannot snkare ${typeof this} to ${typeof given} (${this.toString()}, ${given.toString()})`);
    }
  }
