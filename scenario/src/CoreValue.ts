...
import { snktrollerFetchers, getsnktrollerValue } from './Value/snktrollerValue';
import { snktrollerImplFetchers, getsnktrollerImplValue } from './Value/snktrollerImplValue';
...
import { getsnkValue, snkFetchers } from './Value/snkValue';
...
// TODO: We may want to handle simple values -> snklex values at the parser level
//       This is currently trying to parse simple values as simple or snklex values,
//       and this is because items like `Some` could work either way.
export async function mapValue<T>(
  world: World,
  event: Event,
  simple: (string) => T,
  snklex: (World, Event) => Promise<Value>,
  type: any
): Promise<T> {
  let simpleErr;
  let val;
  ...
  if (typeof event === 'string') {
    try {
      ...
      // Collect the error, but fallback to a snklex expression
      simpleErr = err;
    }
  }

  try {
    val = await snklex(world, event);
  } catch (snklexErr) {
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let issnkKey = '0x' + toBN(newKey).add(toBN(3)).toString(16);
          let issnkStr = await world.web3.eth.getStorageAt(addr.val, issnkKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(issnkStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    `
      ...
      * "Equal given:<Value> expected:<Value>" - Returns true if given values are equal
        ...
        * E.g. "Equal (CToken cZRX snktroller) (snktroller Address)"
    `,
    'Equal',
    [new Arg('given', getCoreValue), new Arg('expected', getCoreValue)],
    async (world, { given, expected }) => new BoolV(expected.snkareTo(world, given))
  ),
  new Fetcher<
      {
        argTypes: StringV[];
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "snktroller ...snktrollerArgs" - Returns snktroller value
    `,
    'snktroller',
    [new Arg('res', getsnktrollerValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: snktrollerFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### snktrollerImpl

      * "snktrollerImpl ...snktrollerImplArgs" - Returns snktroller implementation value
    `,
    'snktrollerImpl',
    [new Arg('res', getsnktrollerImplValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: snktrollerImplFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### CToken
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "snk ...snkArgs" - Returns snk value
    `,
    'snk',
    [new Arg('res', getsnkValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: snkFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Governor
...
let contractFetchers = [
  ...
  { contract: "DeFiDaoLens", implicit: false },
