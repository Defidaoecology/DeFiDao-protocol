...
async function getsnktroller(world: World, cToken: CToken): Promise<AddressV> {
  return new AddressV(await cToken.methods.snktroller().call());
}
...
export function cTokenFetchers() {
  return [
    ...
    new Fetcher<{ cToken: CToken }, AddressV>(`
        ...
        * "CToken <CToken> snktroller" - Returns the cToken's snktroller
          * E.g. "CToken cZRX snktroller"
      `,
      "snktroller",
      [
        new Arg("cToken", getCTokenV)
      ],
      (world, { cToken }) => getsnktroller(world, cToken),
      { namePos: 1 }
    ),
