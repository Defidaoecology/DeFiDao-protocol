...
async function getsnktrollerImplementation(world: World, unitroller: Unitroller): Promise<AddressV> {
  return new AddressV(await unitroller.methods.snktrollerImplementation().call());
}

async function getPendingsnktrollerImplementation(world: World, unitroller: Unitroller): Promise<AddressV> {
  return new AddressV(await unitroller.methods.pendingsnktrollerImplementation().call());
}
    ...
    new Fetcher<{ unitroller: Unitroller }, AddressV>(
      `
        ...
        * "Unitroller Implementation" - Returns the Implementation of Unitroller contract
          * E.g. "Unitroller Implementation" - Returns address of snktrollerImplentation
      `,
      'Implementation',
      [new Arg('unitroller', getUnitroller, { implicit: true })],
      (world, { unitroller }) => getsnktrollerImplementation(world, unitroller)
    ),
    new Fetcher<{ unitroller: Unitroller }, AddressV>(
      `
        ...
        * "Unitroller PendingImplementation" - Returns the pending implementation of Unitroller contract
          * E.g. "Unitroller PendingImplementation" - Returns address of pendingsnktrollerImplementation
      `,
      'PendingImplementation',
      [new Arg('unitroller', getUnitroller, { implicit: true })],
      (world, { unitroller }) => getPendingsnktrollerImplementation(world, unitroller)
    )
  ];
}
