...
async function calculateSeizeTokens(snktroller, cTokenBorrowed, cTokenCollateral, repayAmount) {
  return call(snktroller, 'liquidateCalculateSeizeTokens', [cTokenBorrowed._address, cTokenCollateral._address, repayAmount]);
}
...
describe('snktroller', () => {
  let root, accounts;
  let snktroller, cTokenBorrowed, cTokenCollateral;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    snktroller = await makesnktroller();
    cTokenBorrowed = await makeCToken({snktroller: snktroller, underlyingPrice: 0});
    cTokenCollateral = await makeCToken({snktroller: snktroller, underlyingPrice: 0});
  });
  ...
  describe('liquidateCalculateAmountSeize', () => {
    it("fails if either asset price is 0", async () => {
      ...
      expect(
        await calculateSeizeTokens(snktroller, cTokenBorrowed, cTokenCollateral, repayAmount)
    ...
    it("fails if the repayAmount causes overflow ", async () => {
      expect(
        await calculateSeizeTokens(snktroller, cTokenBorrowed, cTokenCollateral, UInt256Max())
    ...
    it("reverts if it fails to calculate the exchange rate", async () => {
      ...
      await expect(
        send(snktroller, 'liquidateCalculateSeizeTokens', [cTokenBorrowed._address, cTokenCollateral._address, repayAmount])
    ...
    [
      ...
      it(`returns the correct value for ${testCase}`, async () => {
        ...
        await setOraclePrice(cTokenCollateral, collateralPrice);
        await setOraclePrice(cTokenBorrowed, borrowedPrice);
        await send(snktroller, '_setLiquidationIncentive', [liquidationIncentive]);
