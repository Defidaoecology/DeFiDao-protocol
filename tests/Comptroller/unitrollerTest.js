...
describe('Unitroller', () => {
  ...
  describe("constructor", () => {
    it("sets admin to caller and addresses to 0", async () => {
      ...
      expect(await call(unitroller, 'pendingsnktrollerImplementation')).toBeAddressZero();
      expect(await call(unitroller, 'snktrollerImplementation')).toBeAddressZero();
    });
  });
  ...
  describe("_setPendingImplementation", () => {
    ...
    describe("succeeding", () => {
      it("stores pendingsnktrollerImplementation with value newPendingImplementation", async () => {
        await setPending(brains, root);
        expect(await call(unitroller, 'pendingsnktrollerImplementation')).toEqual(brains._address);
      });
  ...
  describe("_acceptImplementation", () => {
    describe("Check caller is pendingsnktrollerImplementation  and pendingsnktrollerImplementation ≠ address(0) ", () => {
      let result;
      beforeEach(async () => {
      ...
      it("does not change current implementation address", async () => {
        expect(await call(unitroller, 'snktrollerImplementation')).not.toEqual(unitroller._address);
      });
    });
    ...
    describe("the brains must accept the responsibility of implementation", () => {
      ...
      it("Store snktrollerImplementation with value pendingsnktrollerImplementation", async () => {
        expect(await call(unitroller, 'snktrollerImplementation')).toEqual(brains._address);
      });

      it("Unset pendingsnktrollerImplementation", async () => {
        expect(await call(unitroller, 'pendingsnktrollerImplementation')).toBeAddressZero();
      });
    ...
    describe("fallback delegates to brains", () => {
      let troll;
      beforeEach(async () => {
        troll = await deploy('EchoTypessnktroller');
        unitroller = await deploy('Unitroller');
