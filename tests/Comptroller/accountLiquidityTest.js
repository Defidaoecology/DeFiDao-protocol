...
describe('snktroller', () => {
  ...
  describe('liquidity', () => {
    it("fails if a price has not been set", async () => {
      ...
      let result = await call(cToken.snktroller, 'getAccountLiquidity', [accounts[1]]);
    ...
    it("allows a borrow up to collateralFactor, but not more", async () => {
      ...
      ({1: liquidity, 2: shortfall} = await call(cToken.snktroller, 'getHypotheticalAccountLiquidity', [user, cToken._address, 0, amount]));
      ...
      ({1: liquidity, 2: shortfall} = await call(cToken.snktroller, 'getAccountLiquidity', [user]));
      ...
      ({1: liquidity, 2: shortfall} = await call(cToken.snktroller, 'getHypotheticalAccountLiquidity', [user, cToken._address, amount, 0]));
      expect(liquidity).toEqualNumber(0);
      expect(shortfall).toEqualNumber(0);
    }, 20000);
    ...
    it("allows entering 3 markets, supplying to 2 and borrowing up to collateralFactor in the 3rd", async () => {
      ...
      const cToken2 = await makeCToken({supportMarket: true, snktroller: cToken1.snktroller, collateralFactor: cf2, underlyingPrice: up2});
      const cToken3 = await makeCToken({supportMarket: true, snktroller: cToken1.snktroller, collateralFactor: cf3, underlyingPrice: up3});
      ...
      ({0: error, 1: liquidity, 2: shortfall} = await call(cToken3.snktroller, 'getAccountLiquidity', [user]));
      expect(error).toEqualNumber(0);
      expect(liquidity).toEqualNumber(collateral);
      expect(shortfall).toEqualNumber(0);

      ({1: liquidity, 2: shortfall} = await call(cToken3.snktroller, 'getHypotheticalAccountLiquidity', [user, cToken3._address, Math.floor(c2), 0]));
      expect(liquidity).toEqualNumber(collateral);
      expect(shortfall).toEqualNumber(0);

      ({1: liquidity, 2: shortfall} = await call(cToken3.snktroller, 'getHypotheticalAccountLiquidity', [user, cToken3._address, 0, Math.floor(c2)]));
      expect(liquidity).toEqualNumber(c1);
      expect(shortfall).toEqualNumber(0);

      ({1: liquidity, 2: shortfall} = await call(cToken3.snktroller, 'getHypotheticalAccountLiquidity', [user, cToken3._address, 0, collateral + c1]));
      expect(liquidity).toEqualNumber(0);
      expect(shortfall).toEqualNumber(c1);

      ({1: liquidity, 2: shortfall} = await call(cToken1.snktroller, 'getHypotheticalAccountLiquidity', [user, cToken1._address, amount1, 0]));
  ...
  describe("getAccountLiquidity", () => {
    it("returns 0 if not 'in' any markets", async () => {
      ...
      const {0: error, 1: liquidity, 2: shortfall} = await call(snktroller, 'getAccountLiquidity', [accounts[0]]);
      expect(error).toEqualNumber(0);
      expect(liquidity).toEqualNumber(0);
      expect(shortfall).toEqualNumber(0);
    });
  ...
  describe("getHypotheticalAccountLiquidity", () => {
    it("returns 0 if not 'in' any markets", async () => {
      ...
      const {0: error, 1: liquidity, 2: shortfall} = await call(cToken.snktroller, 'getHypotheticalAccountLiquidity', [accounts[0], cToken._address, 0, 0]);
      expect(error).toEqualNumber(0);
      expect(liquidity).toEqualNumber(0);
      expect(shortfall).toEqualNumber(0);
    });
    ...
    it("returns collateral factor times dollar amount of tokens minted in a single market", async () => {
      ...
      const {0: error, 1: liquidity, 2: shortfall} = await call(cToken.snktroller, 'getHypotheticalAccountLiquidity', [from, cToken._address, 0, 0]);
      expect(error).toEqualNumber(0);
