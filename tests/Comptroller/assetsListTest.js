...
describe('assetListTest', () => {
  ...
  beforeEach(async () => {
    ...
    snktroller = await makesnktroller({maxAssets: 10});
    allTokens = [OMG, ZRX, BAT, REP, DAI, SKT] = await Promise.all(
      ['OMG', 'ZRX', 'BAT', 'REP', 'DAI', 'sketch']
        .map(async (name) => makeCToken({snktroller, name, symbol: name, supportMarket: name != 'sketch', underlyingPrice: 0.5}))
    );
  });

  describe('_setMaxAssets', () => {
    it("fails if called by a non-admin", async () => {
      expect(await send(snktroller, '_setMaxAssets', [15], {from: customer})).toHaveTrollFailure('UNAUTHORIZED', 'SET_MAX_ASSETS_OWNER_CHECK');
      expect(await call(snktroller, 'maxAssets')).toEqualNumber(10);
    });

    it("succeeds if called by an admin", async() => {
      expect(await send(snktroller, '_setMaxAssets', [15])).toHaveLog('NewMaxAssets', {
          oldMaxAssets: "10",
          newMaxAssets: "15"
        });
      expect(await call(snktroller, 'maxAssets')).toEqualNumber(15);
    });
  });
  ...
  async function checkMarkets(expectedTokens) {
    for (let token of allTokens) {
      ...
      expect(await call(snktroller, 'checkMembership', [customer, token._address])).toEqual(isExpected);
    }
  }

  async function enterAndCheckMarkets(enterTokens, expectedTokens, expectedErrors = null) {
    const {reply, receipt} = await both(snktroller, 'enterMarkets', [enterTokens.map(t => t._address)], {from: customer});
    const assetsIn = await call(snktroller, 'getAssetsIn', [customer]);
  ...
  async function exitAndCheckMarkets(exitToken, expectedTokens, expectedError = 'NO_ERROR') {
    const {reply, receipt} = await both(snktroller, 'exitMarket', [exitToken._address], {from: customer});
    const assetsIn = await call(snktroller, 'getAssetsIn', [customer]);
  ...
  describe('enterMarkets', () => {
    ...
    it("the market must be listed for add to succeed", async () => {
      ...
      await send(snktroller, '_supportMarket', [SKT._address]);
    ...
    it("can enter one + asset cap reached", async () => {
      await send(snktroller, '_setMaxAssets', [1]);
    ...
    describe('reaching the asset cap', () => {
      beforeEach(async () => {
        await send(snktroller, '_setMaxAssets', [3]);
      ...
      it("does not grow if user exactly at asset cap", async () => {
        ...
        await send(snktroller, '_setMaxAssets', [4]);
  ...
  describe('entering from borrowAllowed', () => {
    ...
    it("reverts when called by not a ctoken", async () => {
      await expect(
        send(snktroller, 'borrowAllowed', [BAT._address, customer, 1], {from: customer})
