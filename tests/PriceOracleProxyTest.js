...
describe('PriceOracleProxy', () => {
  ...
  beforeEach(async () => {
    ...
    cEth = await makeCToken({kind: "cether", snktrollerOpts: {kind: "v1-no-proxy"}, supportMarket: true});
    cUsdc = await makeCToken({snktroller: cEth.snktroller, supportMarket: true});
    cSai = await makeCToken({snktroller: cEth.snktroller, supportMarket: true});
    cDai = await makeCToken({snktroller: cEth.snktroller, supportMarket: true});
    cUsdt = await makeCToken({snktroller: cEth.snktroller, supportMarket: true});
    cOther = await makeCToken({snktroller: cEth.snktroller, supportMarket: true});
  ...
  describe("getUnderlyingPrice", () => {
    ...
    it("returns 0 for token without a price", async () => {
      let unlistedToken = await makeCToken({snktroller: cEth.snktroller});
