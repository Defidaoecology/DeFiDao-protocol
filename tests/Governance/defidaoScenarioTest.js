...
describe('snkScenario', () => {
  let root, accounts;
  let snk;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    snk = await deploy('snkScenario', [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await snk.methods.generateCheckpoints(amt, offset).send({from: root, gas: 200000000});
          remaining -= amt;
          offset += amt;
        }

        let result = await snk.methods.getPriorVotes(root, 1).send();
