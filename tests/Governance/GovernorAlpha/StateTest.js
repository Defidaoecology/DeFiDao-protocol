...
describe('GovernorAlpha#state/1', () => {
  let snk, gov, root, acct, delay, timelock;
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorAlpha', [timelock._address, snk._address, root]);
    await send(timelock, "harnessSetAdmin", [gov._address])
    await send(snk, 'transfer', [acct, etherMantissa(4000000)]);
    await send(snk, 'delegate', [acct], { from: acct });
  });
  ...
  it("Canceled", async () => {
    await send(snk, 'transfer', [accounts[0], etherMantissa(4000000)]);
    await send(snk, 'delegate', [accounts[0]], { from: accounts[0] });
    await mineBlock()
    ...
    await send(snk, 'delegate', [root], { from: accounts[0] });
