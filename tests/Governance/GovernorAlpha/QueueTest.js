...
describe('GovernorAlpha#queue/1', () => {
  ...
  describe("overlapping actions", () => {
    it("reverts on queueing overlapping actions in same proposal", async () => {
      ...
      const snk = await deploy('snk', [root]);
      const gov = await deploy('GovernorAlpha', [timelock._address, snk._address, root]);
      const txAdmin = await send(timelock, 'harnessSetAdmin', [gov._address]);

      await enfranchise(snk, a1, 3e6);
      await mineBlock();

      const targets = [snk._address, snk._address];
      const values = ["0", "0"];
    ...
    it("reverts on queueing overlapping actions in different proposals, works if waiting", async () => {
      ...
      await enfranchise(snk, a1, 3e6);
      await enfranchise(snk, a2, 3e6);
      await mineBlock();

      const targets = [snk._address];
      const values = ["0"];
