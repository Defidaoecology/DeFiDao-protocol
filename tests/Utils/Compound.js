...
async function makesnktroller(opts = {}) {
  const {
    root = saddle.account,
    kind = 'unitroller'
  } = opts || {};
  ...
  if (kind == 'bool') {
    return await deploy('Boolsnktroller');
  }

  if (kind == 'false-marker') {
    return await deploy('FalseMarkerMethodsnktroller');
  }

  if (kind == 'v1-no-proxy') {
    const snktroller = await deploy('snktrollerHarness');
    ...
    await send(snktroller, '_setCloseFactor', [closeFactor]);
    await send(snktroller, '_setMaxAssets', [maxAssets]);
    await send(snktroller, '_setPriceOracle', [priceOracle._address]);

    return Object.assign(snktroller, { priceOracle });
  }

  if (kind == 'unitroller-g2') {
    const unitroller = opts.unitroller || await deploy('Unitroller');
    const snktroller = await deploy('snktrollerScenarioG2');
    const priceOracle = opts.priceOracle || await makePriceOracle(opts.priceOracleOpts);
    const closeFactor = etherMantissa(dfn(opts.closeFactor, .051));
    const maxAssets = etherUnsigned(dfn(opts.maxAssets, 10));
    const liquidationIncentive = etherMantissa(1);

    await send(unitroller, '_setPendingImplementation', [snktroller._address]);
    await send(snktroller, '_become', [unitroller._address]);
    mergeInterface(unitroller, snktroller);
  ...
  if (kind == 'unitroller-g3') {
    const unitroller = opts.unitroller || await deploy('Unitroller');
    const snktroller = await deploy('snktrollerScenarioG3');
    const priceOracle = opts.priceOracle || await makePriceOracle(opts.priceOracleOpts);
    const closeFactor = etherMantissa(dfn(opts.closeFactor, .051));
    const maxAssets = etherUnsigned(dfn(opts.maxAssets, 10));
    const liquidationIncentive = etherMantissa(1);
    const snkRate = etherUnsigned(dfn(opts.snkRate, 1e18));
    const snkMarkets = opts.snkMarkets || [];
    const otherMarkets = opts.otherMarkets || [];

    await send(unitroller, '_setPendingImplementation', [snktroller._address]);
    await send(snktroller, '_become', [unitroller._address, snkRate, snkMarkets, otherMarkets]);
    mergeInterface(unitroller, snktroller);
    await send(unitroller, '_setLiquidationIncentive', [liquidationIncentive]);
    await send(unitroller, '_setCloseFactor', [closeFactor]);
    await send(unitroller, '_setMaxAssets', [maxAssets]);
  ...
  if (kind == 'unitroller') {
    ...
    const maxAssets = etherUnsigned(dfn(opts.maxAssets, 10));
    const liquidationIncentive = etherMantissa(1);
    const snk = opts.snk || await deploy('snk', [opts.snkOwner || root]);
    const snkRate = etherUnsigned(dfn(opts.snkRate, 1e18));
    ...
    await send(unitroller, '_setMaxAssets', [maxAssets]);
    await send(unitroller, '_setPriceOracle', [priceOracle._address]);
    await send(unitroller, 'setsnkAddress', [snk._address]); // harness only
    await send(unitroller, '_setsnkRate', [snkRate]);

    return Object.assign(unitroller, { priceOracle, snk });
  }
}
...
async function makeCToken(opts = {}) {
  ...
  const snktroller = opts.snktroller || await makesnktroller(opts.snktrollerOpts);
  ...
  switch (kind) {
    ...
    case 'csnk':
      underlying = await deploy('snk', [opts.snkHolder || root]);
      cDelegatee = await deploy('CsnkLikeDelegate');
      cDelegator = await deploy('CErc20Delegator',
        [
          underlying._address,
          snktroller._address,
      ...
      cToken = await saddle.getContractAt('CsnkLikeDelegate', cDelegator._address);
      break;
  ...
  if (opts.supportMarket) {
    await send(snktroller, '_supportMarket', [cToken._address]);
  }

  if (opts.addsnkMarket) {
    await send(snktroller, '_addsnkMarket', [cToken._address]);
  }

  if (opts.underlyingPrice) {
    const price = etherMantissa(opts.underlyingPrice);
    await send(snktroller.priceOracle, 'setUnderlyingPrice', [cToken._address, price]);
  }

  if (opts.collateralFactor) {
    const factor = etherMantissa(opts.collateralFactor);
    expect(await send(snktroller, '_setCollateralFactor', [cToken._address, factor])).toSucceed();
  }

  return Object.assign(cToken, { name, symbol, underlying, snktroller, interestRateModel });
}
...
async function enterMarkets(cTokens, from) {
  return await send(cTokens[0].snktroller, 'enterMarkets', [cTokens.map(c => c._address)], { from });
}
...
async function setOraclePrice(cToken, price) {
  return send(cToken.snktroller.priceOracle, 'setUnderlyingPrice', [cToken._address, etherMantissa(price)]);
}
