...
async function preMint(cToken, minter, mintAmount, mintTokens, exchangeRate) {
  ...
  await send(cToken.snktroller, 'setMintAllowed', [true]);
  await send(cToken.snktroller, 'setMintVerify', [true]);
...
async function preRedeem(cToken, redeemer, redeemTokens, redeemAmount, exchangeRate) {
  ...
  await send(cToken.snktroller, 'setRedeemAllowed', [true]);
  await send(cToken.snktroller, 'setRedeemVerify', [true]);
  await send(cToken.interestRateModel, 'setFailBorrowRate', [false]);
...
describe('CToken', function () {
  ...
  beforeEach(async () => {
    ...
    cToken = await makeCToken({snktrollerOpts: {kind: 'bool'}, exchangeRate});
  });
  ...
  describe('mintFresh', () => {
    ...
    it("fails if snktroller tells it to", async () => {
      await send(cToken.snktroller, 'setMintAllowed', [false]);
      expect(await mintFresh(cToken, minter, mintAmount)).toHaveTrollReject('MINT_snkTROLLER_REJECTION', 'MATH_ERROR');
    });
  ...
  [redeemFreshTokens, redeemFreshAmount].forEach((redeemFresh) => {
    describe(redeemFresh.name, () => {
      ...
      it("fails if snktroller tells it to", async () =>{
        await send(cToken.snktroller, 'setRedeemAllowed', [false]);
        expect(await redeemFresh(cToken, redeemer, redeemTokens, redeemAmount)).toHaveTrollReject('REDEEM_snkTROLLER_REJECTION');
      });
