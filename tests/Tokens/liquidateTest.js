...
async function preLiquidate(cToken, liquidator, borrower, repayAmount, cTokenCollateral) {
  ...
  await send(cToken.snktroller, 'setLiquidateBorrowAllowed', [true]);
  await send(cToken.snktroller, 'setLiquidateBorrowVerify', [true]);
  await send(cToken.snktroller, 'setRepayBorrowAllowed', [true]);
  await send(cToken.snktroller, 'setRepayBorrowVerify', [true]);
  await send(cToken.snktroller, 'setSeizeAllowed', [true]);
  await send(cToken.snktroller, 'setSeizeVerify', [true]);
  await send(cToken.snktroller, 'setFailCalculateSeizeTokens', [false]);
  ...
  await send(cTokenCollateral.snktroller, 'setCalculatedSeizeTokens', [seizeTokens]);
...
describe('CToken', function () {
  ...
  beforeEach(async () => {
    ...
    cToken = await makeCToken({snktrollerOpts: {kind: 'bool'}});
    cTokenCollateral = await makeCToken({snktroller: cToken.snktroller});
  });
  ...
  describe('liquidateBorrowFresh', () => {
    it("fails if snktroller tells it to", async () => {
      await send(cToken.snktroller, 'setLiquidateBorrowAllowed', [false]);
      expect(
        await liquidateFresh(cToken, liquidator, borrower, repayAmount, cTokenCollateral)
      ).toHaveTrollReject('LIQUIDATE_snkTROLLER_REJECTION', 'MATH_ERROR');
    });

    it("proceeds if snktroller tells it to", async () => {
      expect(
        await liquidateFresh(cToken, liquidator, borrower, repayAmount, cTokenCollateral)
      ).toSucceed();
    });
    ...
    it("fails if calculating seize tokens fails and does not adjust balances", async () => {
      ...
      await send(cToken.snktroller, 'setFailCalculateSeizeTokens', [true]);
      await expect(
        liquidateFresh(cToken, liquidator, borrower, repayAmount, cTokenCollateral)
      ).rejects.toRevert('revert LIQUIDATE_snkTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED');
    ...
    it("fails if repay fails", async () => {
      await send(cToken.snktroller, 'setRepayBorrowAllowed', [false]);
      expect(
        await liquidateFresh(cToken, liquidator, borrower, repayAmount, cTokenCollateral)
    ...
    it("reverts if seize fails", async () => {
      await send(cToken.snktroller, 'setSeizeAllowed', [false]);
      await expect(
        liquidateFresh(cToken, liquidator, borrower, repayAmount, cTokenCollateral)
    ...
    it("reverts if liquidateBorrowVerify fails", async() => {
      await send(cToken.snktroller, 'setLiquidateBorrowVerify', [false]);
      await expect(
        liquidateFresh(cToken, liquidator, borrower, repayAmount, cTokenCollateral)
  ...
  describe('seize', () => {
    ...
    it("fails if seize is not allowed", async () => {
      await send(cToken.snktroller, 'setSeizeAllowed', [false]);
      expect(await seize(cTokenCollateral, liquidator, borrower, seizeTokens)).toHaveTrollReject('LIQUIDATE_SEIZE_snkTROLLER_REJECTION', 'MATH_ERROR');
    });
