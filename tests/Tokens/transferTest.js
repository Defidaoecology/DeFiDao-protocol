const {makeCToken} = require('../Utils/DeFiDao');
...
describe('CToken', function () {
  ...
  describe('transfer', () => {
    ...
    it("rejects transfer when not allowed and reverts if not verified", async () => {
      const cToken = await makeCToken({snktrollerOpts: {kind: 'bool'}});
      await send(cToken, 'harnessSetBalance', [root, 100]);
      expect(await call(cToken, 'balanceOf', [root])).toEqualNumber(100);

      await send(cToken.snktroller, 'setTransferAllowed', [false])
      expect(await send(cToken, 'transfer', [root, 50])).toHaveTrollReject('TRANSFER_snkTROLLER_REJECTION');

      await send(cToken.snktroller, 'setTransferAllowed', [true])
      await send(cToken.snktroller, 'setTransferVerify', [false])
