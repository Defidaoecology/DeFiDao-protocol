...
async function preBorrow(cToken, borrower, borrowAmount) {
  await send(cToken.snktroller, 'setBorrowAllowed', [true]);
  await send(cToken.snktroller, 'setBorrowVerify', [true]);
...
describe('CEther', function () {
  ...
  beforeEach(async () => {
    ...
    cToken = await makeCToken({kind: 'cether', snktrollerOpts: {kind: 'bool'}});
  });
  ...
  describe('borrowFresh', () => {
    ...
    it("fails if snktroller tells it to", async () => {
      await send(cToken.snktroller, 'setBorrowAllowed', [false]);
      expect(await borrowFresh(cToken, borrower, borrowAmount)).toHaveTrollReject('BORROW_snkTROLLER_REJECTION');
    });
    ...
    it("reverts if borrowVerify fails", async() => {
      await send(cToken.snktroller, 'setBorrowVerify', [false]);
    ...
    [true, false].forEach(async (benefactorPaying) => {
      ...
      describe(label, () => {
        ...
        it("fails if repay is not allowed", async () => {
          ...
          expect(await repayBorrowFresh(cToken, payer, borrower, repayAmount)).toHaveTrollReject('REPAY_BORROW_snkTROLLER_REJECTION', 'MATH_ERROR');
        });
        ...
        it("reverts if repayBorrowVerify fails", async() => {
          await send(cToken.snktroller, 'setRepayBorrowVerify', [false]);
  ...
  describe('repayBorrow', () => {
    ...
    it("reverts when repay borrow fresh fails", async () => {
      await send(cToken.snktroller, 'setRepayBorrowAllowed', [false]);
      await expect(repayBorrow(cToken, borrower, repayAmount)).rejects.toRevertWithError('snkTROLLER_REJECTION', "revert repayBorrow failed");
    });
  ...
  describe('repayBorrowBehalf', () => {
    ...
    it("reverts from within repay borrow fresh", async () => {
      await send(cToken.snktroller, 'setRepayBorrowAllowed', [false]);
      await expect(repayBorrowBehalf(cToken, payer, borrower, repayAmount)).rejects.toRevertWithError('snkTROLLER_REJECTION', "revert repayBorrowBehalf failed");
    });
