...
describe.skip('snkWheelFuzzTest', () => {
  // This whole test is fake, but we're testing to see if our equations match reality.

  // First, we're going to build a simple simulator of the DeFiDao protocol
  ...
  let initialState = globals => {
    return {
      ...
      // flywheel & snktroller
      snkSupplySpeed: new bn(1),
      snkSupplyIndex: new bn(1),
      snkSupplyIndexSnapshots: {},
      snkSupplyIndexUpdatedBlock: globals.blockNumber,

      snkBorrowSpeed: new bn(1),
      snkBorrowIndex: new bn(1),
      snkBorrowIndexSnapshots: {},
      snkSupplyIndexUpdatedBlock: globals.blockNumber,

      snkAccruedWithCrank: {}, // naive method, accruing all accounts every block
      snkAccruedWithIndex: {}, // with indices

      activeBorrowBlocks: new bn(0), // # blocks with an active borrow, for which we expect to see snk distributed. just for fuzz testing.
  ...
  let trueUpsnk = (globals, state) => {
    state = accrueInterest(globals, state);

    state = Object.keys(state.snkSupplyIndexSnapshots).reduce(
      (acc, account) => supplierFlywheelByIndex(globals, state, account),
      state
    );

    state = Object.keys(state.snkBorrowIndexSnapshots).reduce(
  ...
  // manual flywheel loops through every account and updates snk accrued mapping
  ...
  let flywheelByCrank = (
    ...
    for (let [account, balance] of Object.entries(balances)) {
      if (isPositive(totalSupply)) {
        snkAccruedWithCrank[account] = get(
          state.snkAccruedWithCrank[account]
        ).plus(
          deltaBlocks
            .times(snkSupplySpeed)
            .times(balance)
            .div(totalSupply)
        );
      }
    ...
    return {
      ...state,
      snkAccruedWithCrank: snkAccruedWithCrank,
    };
  };

  // real deal snk index flywheel™️
  let borrowerFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      totalBorrows,
      borrowBalances,
      snkBorrowIndexUpdatedBlock,
      borrowIndex,
      borrowIndexSnapshots
    } = state;

    let deltaBlocks = globals.blockNumber.minus(snkBorrowIndexUpdatedBlock);
    if (isPositive(totalBorrows)) {
      let scaledTotalBorrows = totalBorrows.div(borrowIndex);
      snkBorrowIndex = snkBorrowIndex.plus(
        snkBorrowSpeed.times(deltaBlocks).div(scaledTotalBorrows)
      );
    }

    let indexSnapshot = snkBorrowIndexSnapshots[account];

    if (
      indexSnapshot !== undefined &&
      snkBorrowIndex.isGreaterThan(indexSnapshot) &&
      ...
      snkAccruedWithIndex[account] = get(snkAccruedWithIndex[account]).plus(
        borrowBalanceNew
          .div(borrowIndex)
          .times(snkBorrowIndex.minus(indexSnapshot))
      );
    }

    return {
      ...state,
      snkBorrowIndexUpdatedBlock: globals.blockNumber,
      snkBorrowIndex: snkBorrowIndex,
      snkBorrowIndexSnapshots: {
        ...state.snkBorrowIndexSnapshots,
        [account]: snkBorrowIndex
      }
    };
  };
  ...
  let supplierFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      snkAccruedWithIndex,
      totalSupply,
      snkSupplyIndexUpdatedBlock
    } = state;

    let deltaBlocks = globals.blockNumber.minus(snkSupplyIndexUpdatedBlock);

    if (isPositive(totalSupply)) {
      snkSupplyIndex = snkSupplyIndex.plus(
        snkSupplySpeed.times(deltaBlocks).div(totalSupply)
      );
    }

    let indexSnapshot = snkSupplyIndexSnapshots[account];
    if (indexSnapshot !== undefined) {
      // if had prev snapshot,  accrue some snk
      snkAccruedWithIndex[account] = get(snkAccruedWithIndex[account]).plus(
        balances[account].times(snkSupplyIndex.minus(indexSnapshot))
      );
    }
    ...
    return {
      ...state,
      snkSupplyIndexUpdatedBlock: globals.blockNumber,
      snkSupplyIndex: snkSupplyIndex,
      snkSupplyIndexSnapshots: {
        ...state.snkSupplyIndexSnapshots,
        [account]: snkSupplyIndex
      },
      snkAccruedWithIndex: snkAccruedWithIndex
    };
  };
  ...
  // assert amount distributed by the crank is expected, that it equals # blocks with a supply * snk speed
  let crankCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.activeSupplyBlocks
      .times(state.snkSupplySpeed)
      .plus(state.activeBorrowBlocks.times(state.snkBorrowSpeed));

    let actual = Object.values(state.snkAccruedWithCrank).reduce(
    ...
    invariantFn(
      ...
      expected,
      actual,
      `crank method distributed snk inaccurately`
    );
  };

  // assert snk distributed by index is the same as amount distributed by crank
  let indexCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.snkAccruedWithCrank;
    let actual = state.snkAccruedWithIndex;
    invariantFn(
      (expected, actual) => {
  ...
  let runEvents = (globals, initState, events) => {
    ...
    return trueUpsnk(globals, state);
  };
