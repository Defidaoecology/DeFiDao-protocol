...
usage:
	npx saddle script -n {network} flywheel:init {
		batch: number || null, // how many borrowrs to claim snk for at once. default 100 (~2.3M gas)
...
let accountRequest = async (network, opts) => {
	...
	let url = `https://api.${stageUrl}DeFiDao.finance/api/v2/account?min_borrow_value_in_eth[value]=0.00000000000000001&network=${network}&page_size=${pageSize}&page_number=1`;
...
let filterInitialized = async (borrowersByCToken) => {
	let res = {}
	let batchSize = 75;
	console.log(`Calling snkBorrowerIndex for borrowers in batches of ${batchSize}...\n`);
	for(let cTokenAddr of Object.keys(borrowersByCToken)) {
		let speed = await call(snktroller, 'snkSpeeds', [cTokenAddr]);
		if (Number(speed) != 0){
			for (let borrowerChunk of getChunks(borrowersByCToken[cTokenAddr], batchSize)) {
				try {
					let indices = await Promise.all(borrowerChunk.map(
						async(borrower) => {
							return await call(snktroller, 'snkBorrowerIndex',[cTokenAddr, borrower])
					}));
...
let claimsnkBatch = async (borrowersByCToken, opts) => {
	for (let cTokenAddr of Object.keys(borrowersByCToken)) {
		...
		for (let chunk of getChunks(borrowers, opts.batch)) {
			if (chunk.length == 0) {
				...
				try {
					let tx = await send(snktroller, 'claimsnk', [
						chunk,
						[cTokenAddr],
						true,
						false,
...
(async () => {
	...
	await claimsnkBatch(unInit, opts);
